...
async def group_message_handler(message: Message, bot: Bot, group: Group) -> None:
    ...
    if profanity_check_result:
        profanity_event_text = messages.TEST_FILTER.format(
            text=message.text,
            word=profanity_check_result
        )
        profanity_event_markup = InlineKeyboardMarkup(inline_keyboard=[
            [
        ...
        profanity_event_message = SendMessage(
            text=profanity_event_text,
            reply_markup=profanity_event_markup
        )
        ...
        await spread_messages(
            admins_chats_ids,
            [profanity_event_message],
            bot
        )
